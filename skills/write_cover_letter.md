...
When the role or company context makes it relevant, the AI-assisted engineering practice at Willow is worth including here. Do not describe it generically as "used AI tools" or "built agents" — pull the specific, credible detail from `background.yaml`'s AI-Assisted Engineering Practice anchor: authored a skill-based code-quality reviewer that runs over every diff before commit to catch redundancy and quality regressions, and built a multi-agent pipeline that generates each feature's test suite using separate analyst, designer, implementer, and adversarial-critic agent roles, so no single agent both writes and judges a test. Name at least one of these two specifics rather than speaking about "AI tooling" in the abstract. Frame it as an engineering discipline with a measurable outcome (reduced cycle time on repetitive work, raised rigour of what shipped) — not enthusiasm for a trend. If the role or the candidate's own initiative is also relevant, note in one clause that the same practice (skill-based diff review, multi-agent test design) carries into the Job Search Agent project, as evidence it's a genuine personal practice rather than a one-off workplace mandate. Only include any of this if it genuinely fits the role.
