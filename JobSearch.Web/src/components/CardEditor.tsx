  ...
  return (
    <div className="rounded-lg border border-gray-100 bg-gray-50">
      <div className="flex items-center justify-between p-3">
        <button onClick={() => setOpen(o => !o)} className="flex-1 text-left text-sm font-medium text-gray-700">
          {summary || "New entry"} {open ? "▲" : "▼"}
        </button>
        <button onClick={onRemove} className="ml-2 text-xs text-red-500 hover:text-red-700">Remove</button>
      </div>
...
export function AddButton({ onClick, children }: { onClick: () => void; children: ReactNode }) {
  return (
    <button onClick={onClick} className="text-sm font-medium text-blue-600 hover:text-blue-700">
      {children}
    </button>
  );
}
