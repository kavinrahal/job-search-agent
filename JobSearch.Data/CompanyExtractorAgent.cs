...
public class CompanyExtractorAgent
{
    ...
    public virtual async Task<string?> ExtractAsync(int userId, string postingText)
    {
        var response = await _client.Messages.Create(new MessageCreateParams
        {
            Model = HaikuModel,
            MaxTokens = 64,
            // Structured fact extraction (a single name, or nothing) — low, consistent
            // temperature instead of the API default (1.0). See ClaudeTemperature. The SDK marks
            // this property [Obsolete] because post-Opus-4.6 models (Sonnet 5, Opus 4.7/4.8)
            // reject any non-default value with a 400 — claude-haiku-4-5 predates that
            // restriction and still honors it.
